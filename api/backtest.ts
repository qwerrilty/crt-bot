 ...
 * Scans all past CRT setups. No Telegram alerts sent.
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (candles.length < 2) return res.status(400).json({ error: `Not enough candles for ${symbol}` })
    ...
    const formatted = setups.map(s => ({
      ...
      // C1 — range candle
      c1Open_UTC:   toUTC(s.c1OpenTime),
      c1Open_PHT:   toPHT(s.c1OpenTime),
      c1Close_UTC:  toUTC(s.c1CloseTime),
      c1Close_PHT:  toPHT(s.c1CloseTime),
      c1High:       s.c1High,
      c1Low:        s.c1Low,
      c1Mid:        s.c1Mid,
      c1RangePct:   s.c1RangePct.toFixed(2) + '%',
      // C2 — sweep candle (CRT confirmed on close)
      c2Open_UTC:   toUTC(s.c2OpenTime),
      c2Open_PHT:   toPHT(s.c2OpenTime),
      c2Close_UTC:  toUTC(s.c2CloseTime),
      c2Close_PHT:  toPHT(s.c2CloseTime),  // ← CRT confirmed here
    ...
    return res.status(200).json({ symbol, candlesScanned: candles.length, setupsFound: setups.length, setups: formatted })
  } catch (e) {
